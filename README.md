...
- **Renamed from stock rEFInd** — binary is `ploader_x64.efi` (was `refind_x64.efi`), config is `ploader.conf` (was `refind.conf`), log is `ploader.log` (was `refind.log`), and on-screen/log text says "Ploader" instead of "rEFInd". Function/variable names inherited from upstream rEFInd (e.g. `StartLoader`, `RefindGuid`) were left as-is; only pearOS-facing branding was renamed.
